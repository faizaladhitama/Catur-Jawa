    ...
    if (has_play > 0):
        ...
        while not gui.getAboard().win_cond():
            ...
            if now == "AI":
                ...
                print(difficult)
                now_time = time.time()
                current_tile, next_tile = gui.getAI().test_minimax(gui.getAboard(), difficult)
                ...
                """
                cont += 1
                time_dif = after_time - now_time
                timetotal += time_dif
                print("Running time :", time_dif)
                print()
                ai_sound.stop()
                pion_put = pygame.mixer.Sound(os.path.join("music", "put_pion.wav"))
                pion_put.play()
